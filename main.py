...
def parse_log_line(line): #function for parsing a single line from log
    ...
    if len(splitted) == 8:
        ...
        return {'event_date':date, 'ip_abon':ip_abon, 'ip_nat':ip_nat, 'url':url, 'ip_server':ip_server, 'protocol_type':protocol_type}
...
def insert_into_db(parsed_data):
    ...
    try:
        ...
        client = Client(host = hostname, user = user, database = database, password = password, port = port)
        
        print('Connected to clickhouse db')

        table_name = config('Clickhouse_table_data')['table_name'] #get table name from config file

        client.execute('INSERT INTO {0} (event_date, ip_server, url, ip_nat, ip_abon, protocol_type) VALUES'.format(table_name), parsed_data) #execute command for inserting data
